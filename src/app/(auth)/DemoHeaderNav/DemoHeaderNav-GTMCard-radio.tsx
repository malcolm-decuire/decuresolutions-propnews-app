  ...
  ({ color, title, icon, description, children, className, ...props }, ref) => {
    ...
    return (
      <Card
        ...
        {...props}
      >
        <CardBody className="flex h-full flex-row items-start gap-3 p-4 text-left">
          ...
          <div className="flex flex-col">
            <p className="text-medium text-left">{title}</p>
            <p className="text-small text-default-400 text-left">{description || children}</p>
          </div>
        </CardBody>
      </Card>
    );
