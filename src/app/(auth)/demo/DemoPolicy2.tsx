...
export default function DemoPolicy2(props: CardProps) {
  ...
  const content = isOpen ? (
    <div className="h-full w-full items-start justify-center overflow-scroll px-4 pb-24 pt-20 bg-gray-900 text-white">
      ...
      <ul className="flex flex-col gap-1">
        <li>
          ...
          SQL Joins: Projection policies limit what data can be included in join operations. For instance, you might only allow hashed IDs and aggregated counts to appear in the final dataset.
          </Link>
        </li>
        <br/>
        <li>
          <Link className="text-default-400" href="#" size="sm">
          Matching Accuracy: By reducing the number of columns visible for analysis, these policies may decrease the precision of matching algorithms but enhance security.
          </Link>
        </li>
        <br/>
        <li>
          <Link className="text-default-400" href="#" size="sm">
          Cross-party Transparency: Ensures all participating parties know and agree on what data is projected in the final output.
          </Link>
        </li>
        <br/>
      </ul>
    ...
    <ul>
      <li className="flex items-center gap-1">
        ...
        <p className="text-small text-default-500">Data Minimization</p>
      </li>
      <li className="flex items-center gap-1">
        <Icon className="text-default-600" icon="ci:check" width={24} />
        <p className="text-small text-default-500">Privacy Compliance</p>
      </li>
    </ul>
  );
  ...
  return (
    <Card {...props} className="relative w-full max-w-[400px] pb-[120px]">
      ...
      <CardBody className="relative min-h-[300px] bg-gradient-to-br from-content1 to-default-100/50 p-8 before:inset-0 before:h-full before:w-full before:content-['']">
        <h1 className="mb-4 text-default-400">FOUNDATION #2</h1>
