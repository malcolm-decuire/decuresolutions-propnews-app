...
export default function DemoPolicy4(props: CardProps) {
  ...
  const content = isOpen ? (
    <div className="h-full w-full items-start justify-center overflow-scroll px-4 pb-24 pt-20 bg-gray-900 text-white">
      ...
      <ul className="flex flex-col gap-1">
        <li>
          ...
          SQL Joining: Columns involved in joining operations (e.g., hashed email, customer ID) may be accessible only under strict conditions, affecting how easily datasets can be merged.
          </Link>
        </li>
        <br/>
        <li>
          <Link className="text-default-400" href="#" size="sm">
          Projection & Aggregation: Policies might enforce that specific columns can only appear in aggregated or anonymized forms in the final output.
          </Link>
        </li>
        <br/>
        <li>
    ...
    <ul>
      <li className="flex items-center gap-1">
        ...
        <p className="text-small text-default-500">Sensitive Data Handling</p>
      </li>
      <li className="flex items-center gap-1">
        <Icon className="text-default-600" icon="ci:check" width={24} />
        <p className="text-small text-default-500">Fine-grained Control</p>
      </li>
    </ul>
  );
  ...
  return (
    <Card {...props} className="relative w-full max-w-[400px] pb-[120px]">
      ...
      <CardBody className="relative min-h-[300px] bg-gradient-to-br from-content1 to-default-100/50 p-8 before:inset-0 before:h-full before:w-full before:content-['']">
        <h1 className="mb-4 text-default-400">FOUNDATION #4</h1>
